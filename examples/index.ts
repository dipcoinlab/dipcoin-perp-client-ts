...
// Print SOL + Solana USDC + Sui Bank balances for the connected Solana wallet.
async function showSolanaBalances(sdk: DipCoinPerpSDK) {
  logSection("Solana Balances");
  const res = await sdk.getChainBalances();
  if (res.status && res.data) {
    console.log("SOL (native):", res.data.sui);
    console.log("USDC (Solana wallet):", res.data.usdc);
    console.log("Bank (Sui exchange):", res.data.bank);
  } else {
...
async function runSolanaDemo(privateKey: string, network: Network) {
  ...
  await showSolanaBalances(sdk);

  // CCTP deposit / relayer withdraw (opt-in).
  await maybeSolanaDeposit(
    sdk,
    boolEnv("RUN_SOLANA_DEPOSIT"),
    numberEnv("SOLANA_DEPOSIT_AMOUNT", 10)
  );
...
async function main() {
  ...
  if (stringEnv("CHAIN", "sui").toLowerCase() === "solana") {
